...
public class MusicController {
    ...
    public void pick(Music musicCondition, StompHeaderAccessor accessor) {
        log.info("收到点歌请求: {},{}", musicCondition.getName(), musicCondition.getId());
        ...
        Chat chat = new Chat();
        chat.setContent("点歌 " + musicCondition.getName());
        ...
        Music pick = musicService.getMusic(musicCondition);
        boolean isNull = null == pick || null == pick.getUrl() || (null == pick.getId() && null == pick.getUrl() && null == pick.getDuration());
        if (isNull) {
            log.info("点歌失败, 可能音乐不存在, 关键字: {}, 即将向客户端反馈点歌失败消息", musicCondition.getName());
            ...
            log.info("点歌失败, 音乐: {} 已被拉黑, 关键字: {}, 即将向客户端反馈点歌失败消息", pick.getId(), musicCondition.getName());
            ...
            log.info("点歌失败, 音乐: {} 已在播放列表中, 关键字: {}, 即将向客户端反馈点歌失败消息", pick.getId(), musicCondition.getName());
            sessionService.send(MessageType.NOTICE, Response.failure((Object) null, "点歌失败, 已在播放列表"), houseId);
        } else {
            log.info("点歌成功, 音乐: {}, 时长: {}, 链接: {}, 房间号: {} , 即将向客户端广播消息以及列表", pick.getName(), pick.getDuration(), pick.getUrl(), houseId);
            musicService.toPick(sessionId, pick, houseId, musicCondition.getSource());
...
//            sessionService.send(sessionId, MessageType.NOTICE, Response.failure((Object) null, "你没有权限"));
    ...
    public void search(Music music, HulkPage hulkPage, StompHeaderAccessor accessor) {
        ...
        Page<List<Music>> page = musicService.searchMusic(music, hulkPage);
    ...
    public void searchsonglist(SongList songList, HulkPage hulkPage, StompHeaderAccessor accessor) {
        ...
        Page<List<SongList>> page = musicService.searchSongList(songList, hulkPage);
    ...
    public void searchuser(MusicUser musicUser, HulkPage hulkPage, StompHeaderAccessor accessor) {
        ...
        Page<List<MusicUser>> page = musicService.searchMusicUser(musicUser, hulkPage);
